...
export default function Dropdown({ options = [], selected, setSelected }) {
    ...
    return (
        <div className="relative text-center w-full ">
            <button
                className="flex w-50 justify-between items-center bg-[#0e1722] border border-[#273246] text-center text-white rounded-[5px] px-4 py-2"
                onClick={() => setIsOpen(!isOpen)}
            >
                <span className="w-25">{selected}</span>
                <ChevronDownIcon
